...
def animate_embedding_3d_learnable_siren(data, encoder_path, input_dim, hidden_dim, output_dim,
    ...
    duplicates_mask = torch.any(
        (embeddings.unsqueeze(1) == embeddings.unsqueeze(0)).all(dim=-1).triu(1), dim=1
    )

    # Print indices of duplicated embeddings
    duplicate_indices = torch.nonzero(duplicates_mask).squeeze()
    print("Duplicate embeddings at indices:", duplicate_indices.tolist())
